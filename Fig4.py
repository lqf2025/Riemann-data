...
def add_missing_axes_edges(ax, color='#555555', lw=0.8):
    ...
    ax.plot([x1, x1], [5, 5], [z0, z1], color=color, lw=lw)
...
def tscan_F_from_npz(exp_npz):
    ...
    abs_std = exp_npz["abs_std"]

    F_ref = (-np.log(abs_mean) / 4.0).T.reshape(-1)
    F_std_ref = (abs_std / (4.0 * abs_mean)).T.reshape(-1)
    ...
    if "F_std" in exp_npz and np.allclose(exp_npz["F_std"], F_std_ref):
        F_std = exp_npz["F_std"]
    else:
        print("warning: t-scan F_std in NPZ is inconsistent with delta-method value; using recomputed values")
        F_std = F_std_ref
...
def plot_L2_surface_on_axis(ax, N):
    ...
    ax.set_xticklabels(
        [f"{x:.2f}" for x in np.arange(0.0, BETA_MAX + 0.01, 0.25)],
...
F_m_05, F_std_05 = tscan_F_from_npz(exp_05)
lF_05 = F_std_05.copy()
uF_05 = F_std_05.copy()

F_m_03, F_std_03 = tscan_F_from_npz(exp_03)
lF_03 = F_std_03.copy()
uF_03 = F_std_03.copy()
...
std_F_exp_type3 = exp_beta["F_std"]
...
ax_b3.set_zticklabels(['0', '0.5', '1'], fontsize=FONT_3D_TICK)
ax_a3.set_zticks([0.0, 0.5, 1.0, 1.5])
ax_a3.set_zticklabels(["0", "0.5", "1", "1.5"], fontsize=FONT_3D_TICK)
ax_c3.set_zticks([0, 0.2, 0.4, 0.6])
ax_c3.set_zticklabels(["0", "0.2", "0.4", "0.6"], fontsize=FONT_3D_TICK)
...
ax_d2.set_yticklabels(["0", "0.5", "1", "1.5"])
